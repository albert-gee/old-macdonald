...
The **ESP Matter Controller Example** from
the [ESP-Matter SDK](Espressif.md#esp-matter-solution) is a [Matter Controller](Matter.md#controllers) implementation
that supports the following features of the standard Matter controller:
...
This section demonstrates how to build and run the ESP Matter Controller example.

Prerequisites:

- [ESP-IDF development environment](ESP-IDF-Setup.md) is set up.
- [Matter development environment](Matter-Interface.md) is set up.
- [ESP32-C6-DevKitM-1](Espressif.md#hardware) or [ESP Thread Border Router](Espressif.md#hardware) board
  is available.
...
Set the target device based on the development board:

- **esp32c6** for ESP32-C6:
  ```Bash
  idf.py set-target esp32c6
  ```
- **esp32s3** for Thread Border Router since it uses the ESP32-S3 chip:
  ```Bash
  idf.py -D SDKCONFIG_DEFAULTS="sdkconfig.defaults.otbr" set-target esp32s3
  ```

### Step 4: Configure the Project (ESP32-C6 only) {collapsible="true"}

The following steps are applied **only to the ESP32-C6 board**:
...
- **Change the project configuration**:
  ...
  ```
    ...
    - Navigate to `Component config -> ESP Matter -> Device Configuration` and select `ESP32-C6 DevKitM-1`.
    - Switch from 4MB to 8MB Flash. Navigate to `Serial Flasher Config -> Flash Size`.
    - Disable Matter Server: `Component config -> ESP Matter -> Enable Matter Server`

  A **Matter server** is the software layer that enables a device to function as a Matter node, responding to commands,
  managing data attributes, and communicating within a Matter network.
...
- **Edit partition.csv**:
  ...
  Change the partition table to the following:
  ...
  paa_cert, data, spiffs,  ,          0x100000 
  ```
  {collapsible="true" collapsed-title="partition.csv"}

- **Edit Project's CMakeLists.txt**:

  ```Bash
  nano CMakeLists.txt
  ```

  The following changes are made to the `CMakeLists.txt` file:
...
- Edit Matter Component's CMakeLists {collapsible="true"}

  This file configures how the **Matter component** is built on ESP32.
  
  ```Bash
  nano $ESP_MATTER_PATH/connectedhomeip/connectedhomeip/config/esp32/components/chip/CMakelists.txt
  ```
  
  Fix the path to **libble_app.a** which provides the ESP32-C6 Bluetooth Support by removing
  ` OR (target_name STREQUAL "esp32c6")` from the `if` block and create a new `elseif` block:
...
```Bash
# Note: this example on ESP32-C6 can run either Wi-Fi or Thread, not 
both at the same time. ESP Thread Border Router can run both.
